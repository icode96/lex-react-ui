export const chatWindowWrapper: string
export const chatInputHolder: string
export const chatInputField: string
export const chatSendMessage: string
export const chatListHolder: string
export const chatBlock: string
export const them: string
export const me: string
interface Namespace {
	"chatWindowWrapper": string,
	"chat-window-wrapper": string,
	"chatInputHolder": string,
	"chat-input-holder": string,
	"chatInputField": string,
	"chat-input-field": string,
	"chatSendMessage": string,
	"chat-send-message": string,
	"chatListHolder": string,
	"chat-list-holder": string,
	"chatBlock": string,
	"chat-block": string,
	"them": string,
	"me": string,
}
declare const chatWindow: Namespace
export default chatWindow
